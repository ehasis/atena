...
void CVeiculo::CarregarArquivoDados(const char *arquivo)
{
	...
	DescarregarArquivoDados();
	m_dat_arquivo.Abrir(arquivo);
	//if (!m_dat_arquivo) Erro("Código do erro:", "0900");
}
...
void CVeiculo::Iniciar(int tipo, int x, int y)
{
	...
	m_largura = m_dat_arquivo.ObterBitmap(m_tipo)->w;
	m_altura  = m_dat_arquivo.ObterBitmap(m_tipo)->h;
...
void CVeiculo::Desenhar(CTela &tela, int x_real, int y_real)
{
	...
	switch(m_status)
	{
		case eVeiculoNormal:
			...
			tela.RotateSprite(eCamadaObjetos, m_dat_arquivo.ObterBitmap(m_tipo), m_x - x_real, m_y - y_real, itofix(m_angulo));
