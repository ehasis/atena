...
*   - Criação da função SuavizarBitmap()
*  
*  Diego em 13/02/2002
...
{
	_tela.Rect(eCamadaEfeitos, x,   y,   x + l,                   y + 8, makecol(255,255,255));
	_tela.RectFill(eCamadaEfeitos, x,   y,   x + l - 1,               y + 7, makecol(0,0,0));
	_tela.RectFill(eCamadaEfeitos, x+1, y+1, x + ((percent*l)/100)-1, y + 7, makecol(255,(percent*255)/100,0));
}

void barra_progresso_atena2(CTela &_tela, int x, int y, int l, int percent)
{
	_tela.RectFill(eCamadaEfeitos, x, y, x + l,                 y + 4, makecol(0,0,0));
	_tela.RectFill(eCamadaEfeitos, x, y, x + ((percent*l)/100), y + 4, makecol(255,(percent*255)/100,0));
}
...
void barra_progresso_zeus(CTela &_tela, int x1, int y1, int x2, int y2, int maximo, int valor_anterior, int valor_atual, int cor)
{
	...
	if(((valor_atual / ((maximo / 100) + 1))) == 1 || (valor_atual / ((maximo / 100) + 1)) != (valor_anterior / ((maximo / 100) + 1)))
	{
		...
		_tela.Rect(eCamadaEfeitos, x1, y1, x2, y2, makecol(255, 255, 255));
		_tela.RectFill(eCamadaEfeitos, x1 + 1,   y1 + 1,   x2 - 1, y2 - 1, makecol(0, 0, 0));
		_tela.RectFill(eCamadaEfeitos, x1 + 1, y1 + 1, x1 + (((x2 - x1) / 100) * pos), y2 - 1, cor);
	}
}
