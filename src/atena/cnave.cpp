...
void CNave::Iniciar(int tipo, int x, int y)
{
	...
	m_armas.Adicionar();
	m_armas.Obter().Iniciar(eArmaNave, x, y);
...
void CNave::Desenhar(CTela & tela, int x_real, int y_real)
{
	...
	{
		case eNaveEscudo:
			tela.DrawSprite(eCamadaObjetos, m_dat_arquivo.Bitmap(ESCUDO), m_x - x_real, m_y - x_real);
			m_status = eNaveNormal;
		...
		case eNaveRenacer:
			tela.DrawSprite(eCamadaObjetos, m_dat_arquivo.Bitmap(EXPLOSAO), m_x - x_real, m_y - x_real);
			break;
	}
