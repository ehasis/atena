...
void CEditor::Iniciar()
{
	...
	ChecarArquivo(ARQUIVO_IMAGEM_DAT);
	ChecarArquivo(ARQUIVO_BONUS_DAT);
	ChecarArquivo("fases\\novo.dft");
	ChecarArquivo("fases\\novo.bmp");
	...
	CAlien::CarregarArquivoDados(ARQUIVO_ALIENS_DAT);
	CConstrucao::CarregarArquivoDados(ARQUIVO_CONSTRUCOES_DAT);
	CVeiculo::CarregarArquivoDados(ARQUIVO_VEICULOS_DAT);
	...
	m_dat_imagem.Abrir(ARQUIVO_IMAGEM_DAT);
	if (!m_dat_imagem.Aberto())
	{
		allegro_message("errro!!");
	}
	...
	m_botao_virar_h.Iniciar(ARQUIVO_IMAGEM_DAT,		"BOTAO_H_FLIP",  0, NULL, BOTAO_H_FLIP_X, BOTAO_H_FLIP_Y);
	m_botao_virar_v.Iniciar(ARQUIVO_IMAGEM_DAT,		"BOTAO_V_FLIP",  0, NULL, BOTAO_V_FLIP_X, BOTAO_V_FLIP_Y);
	m_botao_rotacionar.Iniciar(ARQUIVO_IMAGEM_DAT,	"BOTAO_ROTACAO", 0, NULL, BOTAO_ROTACAO_X, BOTAO_ROTACAO_Y);
	m_botao_apagar.Iniciar(ARQUIVO_IMAGEM_DAT,		"BOTAO_APAGAR",  0, NULL, BOTAO_APAGAR_X, BOTAO_APAGAR_Y);
	m_botao_inc_pincel.Iniciar(ARQUIVO_IMAGEM_DAT,	"BOTAO_INC_PINCEL",  0, NULL, BOTAO_INC_PINCEL_X, BOTAO_INC_PINCEL_Y);
	m_botao_dec_pincel.Iniciar(ARQUIVO_IMAGEM_DAT,	"BOTAO_DEC_PINCEL",  0, NULL, BOTAO_DEC_PINCEL_X, BOTAO_DEC_PINCEL_Y);
	...
	lad.v_flip = 0;
	lad.angulo = 0;
	m_ladrilho.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_imagem.Bitmap(LAD_BRANCO));
	m_ladrilho_sel_esquerdo.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_imagem.Bitmap(LAD_BRANCO));
	lad.x = LADRILHO_SEL_DIREITO_X;
	lad.y = LADRILHO_SEL_DIREITO_Y;
	m_ladrilho_sel_direito.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_imagem.Bitmap(LAD_BRANCO));
	DesenharTela();
	show_mouse(screen);
...
void CEditor::Finalizar()
{
	...
	m_dat_aliens.Fechar();
 	m_dat_construcoes.Fechar();
	m_dat_veiculos.Fechar();
		...
		case eAlien:
				for(x = 0; x < MENU_OBJ_LARGURA_LADRILHO; x++)
				{
					for(y = 0; y < MENU_OBJ_ALTURA_LADRILHO; y++)
					{
						...
						m_menu_obj_ladrilhos[x] [y].Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(num_obj));
						...
						if(num_obj > 12)
						{
							if(modo)
							{
								...
								m_ladrilho.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(0));
								// Ladrilhos selecionados
								m_ladrilho_sel_esquerdo.Finalizar();
								m_ladrilho_sel_esquerdo.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(0));
								lad.x = LADRILHO_SEL_DIREITO_X;
								lad.y = LADRILHO_SEL_DIREITO_Y;
								m_ladrilho_sel_direito.Finalizar();
								m_ladrilho_sel_direito.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(2));
		...
		case eConstrucao:
				for(x = 0; x < MENU_OBJ_LARGURA_LADRILHO; x++)
				{
					for(y = 0; y < MENU_OBJ_ALTURA_LADRILHO; y++)
					{
						...
						m_menu_obj_ladrilhos[x] [y].Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap(num_obj));
						m_menu_obj_ladrilhos[x] [y].Desenhar(m_tela, 0, 0);
						m_tela.Rect(eCamadaFundo, m_menu_obj_ladrilhos[x][y].ObterX(), m_menu_obj_ladrilhos[x][y].ObterY(), m_menu_obj_ladrilhos[x][y].ObterX() + LADRILHO_LARGURA, m_menu_obj_ladrilhos[x][y].ObterY() + LADRILHO_ALTURA, makecol(100, 100, 100));
						...
						if(num_obj > 5)
						{
							if(modo)
							{
								...
								// Ladrilho auxiliar
								m_ladrilho.Finalizar();
								m_ladrilho.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap(0));
								// Ladrilhos selecionados
								m_ladrilho_sel_esquerdo.Finalizar();
								m_ladrilho_sel_esquerdo.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap(0));
								lad.x = LADRILHO_SEL_DIREITO_X;
								lad.y = LADRILHO_SEL_DIREITO_Y;
								m_ladrilho_sel_direito.Finalizar();
								m_ladrilho_sel_direito.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap(1));
			...
			case eVeiculo:
				for(x = 0; x < MENU_OBJ_LARGURA_LADRILHO; x++)
				{
					for(y = 0; y < MENU_OBJ_ALTURA_LADRILHO; y++)
					{
						...
						m_menu_obj_ladrilhos[x] [y].Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap(num_obj));
						m_menu_obj_ladrilhos[x] [y].Desenhar(m_tela, 0, 0);
						m_tela.Rect(eCamadaFundo, m_menu_obj_ladrilhos[x][y].ObterX(), m_menu_obj_ladrilhos[x][y].ObterY(), m_menu_obj_ladrilhos[x][y].ObterX() + LADRILHO_LARGURA, m_menu_obj_ladrilhos[x][y].ObterY() + LADRILHO_ALTURA, makecol(100, 100, 100));
						...
						if(num_obj > 0)
						{
							if(modo)
							{
								...
								// Ladrilho auxiliar
								m_ladrilho.Finalizar();
								m_ladrilho.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap(0));
								// Ladrilhos selecionados
								m_ladrilho_sel_esquerdo.Finalizar();
								m_ladrilho_sel_esquerdo.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap(0));
								lad.x = LADRILHO_SEL_DIREITO_X;
								lad.y = LADRILHO_SEL_DIREITO_Y;
								m_ladrilho_sel_direito.Finalizar();
								m_ladrilho_sel_direito.Iniciar(lad, LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap(0));
...
bool CEditor::AbrirFase()
{
	...
	if((file_select_ex("Selecione o m_arquivo do mapa:", arquivo_selecionado, "map", sizeof(arquivo_selecionado), DIALOGO_LARGURA, DIALOGO_ALTURA)) != NULL)
	{
		...
		//strcpy(m_arquivo_fase, strstr(arquivo_selecionado, "fases\\");
...
bool CEditor::AbrirFundo()
{
	...
	if((file_select_ex("Selecione o m_arquivo bitmap para o fundo:", arquivo_selecionado, "bmp", sizeof(arquivo_selecionado), DIALOGO_LARGURA, DIALOGO_ALTURA)) != NULL)
	{
		...
		strcpy(m_arquivo_fundo, strstr(arquivo_selecionado, "fases\\"));
...
{
	m_dat_aliens.Abrir(ARQUIVO_ALIENS_DAT);
...
{
	m_dat_construcoes.Abrir(ARQUIVO_CONSTRUCOES_DAT);
	m_construcao_sel_esquerdo.Iniciar(0, 0, 0);
	m_construcao_sel_direito.Iniciar(1, 0, 0);
...
{
	m_dat_veiculos.Abrir(ARQUIVO_VEICULOS_DAT);
...
void CEditor::DesenharTela()
{
	...
	// Menu Superior
	if(!m_flag_fase) m_tela.RectFill(eCamadaFundo, MAPA_EDITOR_X1, MAPA_EDITOR_Y1, MAPA_EDITOR_X2, MAPA_EDITOR_Y2, makecol(0, 0, 0));

	m_tela.RectFill(eCamadaFundo, MENU_ARQUIVO_X1, 0, SCREEN_W, MENU_ALTURA, makecol(0, 0, 0));
	m_tela.Escrever(eCamadaFundo, "Arquivo", MENU_ARQUIVO_X1, 0, makecol(255, 255, 255));
	m_tela.Escrever(eCamadaFundo, "Objetos", MENU_OBJETOS_X1, 0, makecol(255, 255, 255));
	m_tela.Escrever(eCamadaFundo, "Exibir", MENU_EXIBIR_X1, 0, makecol(255, 255, 255));
	m_tela.Escrever(eCamadaFundo, "Ferramentas", MENU_FERRAMENTAS_X1, 0, makecol(255, 255, 255));

	// Display das coordenadas do mapa da m_fase
	if(m_flag_fase)
	{
		sprintf(buf, "[x: %4i][y: %4i]", m_fase.ObterX1Fonte(), m_fase.ObterY1Fonte());
		m_tela.Escrever(eCamadaFundo, buf, 495, 1, makecol(255, 255, 255));
	}
	...
	m_botao_virar_h.Desenhar(m_tela, eCamadaFundo);
	m_botao_virar_v.Desenhar(m_tela, eCamadaFundo);
	m_botao_rotacionar.Desenhar(m_tela, eCamadaFundo);
	m_botao_apagar.Desenhar(m_tela, eCamadaFundo);
	m_botao_inc_pincel.Desenhar(m_tela, eCamadaFundo);
	m_botao_dec_pincel.Desenhar(m_tela, eCamadaFundo);
	sprintf(buf, "%i", m_tam_pincel_ladrilho);
	m_tela.Escrever(eCamadaFundo, buf, BOTAO_INC_PINCEL_X + 38, BOTAO_INC_PINCEL_Y + 11, makecol(0, 255, 0));
	...
	if(m_flag_fase)
	{
		...
		m_tela.Escrever(eCamadaFundo, buf, BOTAO_INC_PINCEL_X + 140, BOTAO_APAGAR_Y + 5, makecol(0, 255, 0));
	}
...
void CEditor::DesenharQuadrado()
{
	...
	if(m_flag_apagar)
	{
		m_tela.RectFill(eCamadaFundo, m_zeus_mouse_x - (LADRILHO_LARGURA / 2), m_zeus_mouse_y - (LADRILHO_ALTURA / 2), m_zeus_mouse_x + (LADRILHO_LARGURA / 2), m_zeus_mouse_y + (LADRILHO_ALTURA / 2), makecol(128, 128, 128));
		...
		&& m_exibir_obj == eVeiculo))
		{
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (LADRILHO_LARGURA / 2), m_zeus_mouse_y - (LADRILHO_ALTURA / 2), m_zeus_mouse_x + (LADRILHO_LARGURA / 2), m_zeus_mouse_y + (LADRILHO_ALTURA / 2), makecol(255, 0, 0));
		}
		else
		{
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (LADRILHO_LARGURA / 2), m_zeus_mouse_y - (LADRILHO_ALTURA / 2), m_zeus_mouse_x + (LADRILHO_LARGURA / 2), m_zeus_mouse_y + (LADRILHO_ALTURA / 2), makecol(255, 255, 255));
		}
	}
	else
	if(m_exibir_obj == eAlien)
	{
		if(m_fase.ObterAliens().ChecarColisao(m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() - (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() - (m_alien_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() + (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() + (m_alien_sel_esquerdo.ObterAltura() / 2)))
		{
			...
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_alien_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_alien_sel_esquerdo.ObterAltura() / 2), makecol(255, 0, 0));
		}
		else
		{
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_alien_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_alien_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_alien_sel_esquerdo.ObterAltura() / 2), SOBRE_LADRILHO_COR);
		}
	}
	else
	if(m_exibir_obj == eConstrucao)
	{
		if(m_fase.ChecarColisaoConstrucoes(m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() - (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() - (m_construcao_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() + (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() + (m_construcao_sel_esquerdo.ObterAltura() / 2)))
		{
			...
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_construcao_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_construcao_sel_esquerdo.ObterAltura() / 2), makecol(255, 0, 0));
		}
		else
		{
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_construcao_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_construcao_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_construcao_sel_esquerdo.ObterAltura() / 2), SOBRE_LADRILHO_COR);
		}
	}
	else
	if(m_exibir_obj == eVeiculo)
	{
		if(m_fase.ObterVeiculos().ChecarColisao(m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() - (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() - (m_veiculo_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x - 97 + m_fase.ObterX1Fonte() + (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - 11 + m_fase.ObterY1Fonte() + (m_veiculo_sel_esquerdo.ObterAltura() / 2)))
		{
			...
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_veiculo_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_veiculo_sel_esquerdo.ObterAltura() / 2), makecol(255, 0, 0));
		}
		else
		{
			m_tela.Rect(eCamadaFundo, m_zeus_mouse_x - (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y - (m_veiculo_sel_esquerdo.ObterAltura() / 2), m_zeus_mouse_x + (m_veiculo_sel_esquerdo.ObterLargura() / 2), m_zeus_mouse_y + (m_veiculo_sel_esquerdo.ObterAltura() / 2), SOBRE_LADRILHO_COR);
		}
	}
	else
	if(m_exibir_obj == eFundo)
	{
		m_tela.Rect(eCamadaFundo, (m_fase.ObterLadrilho(x, y).ObterX() - (LADRILHO_LARGURA * (m_tam_pincel_ladrilho / 2)) + MAPA_EDITOR_X1 - m_fase.ObterX1Fonte()), (m_fase.ObterLadrilho(x, y).ObterY() - (LADRILHO_ALTURA * (m_tam_pincel_ladrilho / 2)) + MAPA_EDITOR_Y1 -  m_fase.ObterY1Fonte()), (m_fase.ObterLadrilho(x, y).ObterX() + (LADRILHO_LARGURA * ((m_tam_pincel_ladrilho / 2) + 1)) + MAPA_EDITOR_X1 - m_fase.ObterX1Fonte()) - 1, (m_fase.ObterLadrilho(x, y).ObterY() + (LADRILHO_ALTURA * ((m_tam_pincel_ladrilho / 2) + 1)) + MAPA_EDITOR_Y1 - m_fase.ObterY1Fonte()) - 1, SOBRE_LADRILHO_COR);
	}
}
...
bool CEditor::VerificarMenuExibir()
{
	...
	if(m_zeus_mouse_x > MENU_EXIBIR_X1  && m_zeus_mouse_x < MENU_EXIBIR_X2)
	{
		...
		switch(opcao)
		{
			case 1:if (!m_dat_construcoes.Aberto())
					{
						AbrirConstrucoes();
					}
					break;

			case 2:	if (!m_dat_aliens.Aberto())
					{
						AbrirAliens();
					}
					break;
			...
			case 5: if(!m_dat_veiculos.Aberto())
					{
						AbrirVeiculos();
					}
					break;
	...
	if(mouse_b & 1)
	{
		...
		switch(m_exibir_obj)
		{
			case eConstrucao:
				...
				m_ladrilho_sel_esquerdo.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap((x * 15) + y));
			...
			case eAlien:
				...
				m_ladrilho_sel_esquerdo.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(((x * 15) + y) * 2));
			...
			case eVeiculo:
				...
				m_ladrilho_sel_esquerdo.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap((x * 15) + y));
				// Este calculo { ((x * 15) + y) } informa o m_tipo de construcao
	...
	if(mouse_b & 2)
	{
		...
		switch(m_exibir_obj)
		{
			case eConstrucao:
				...
				m_ladrilho_sel_direito.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_construcoes.Bitmap(((x * 15) + y) * 2));
				// Este calculo { ((x * 15) + y) } informa o m_tipo de construcao
			...
			case eAlien:
				// Este calculo { ((x * 15) + y) * 2 } localiza o bitmap do alien no m_arquivo dat
				m_ladrilho_sel_direito.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_aliens.Bitmap(((x * 15) + y) * 2));
				// Este calculo { ((x * 15) + y) } informa o m_tipo de alien
			...
			case eVeiculo:
				// Este calculo { ((x * 15) + y) * 2 } localiza o bitmap do construcao no m_arquivo dat
				m_ladrilho_sel_direito.Iniciar(m_menu_obj_ladrilhos[x][y].ObterTLadrilho(), LADRILHO_LARGURA, LADRILHO_ALTURA, m_dat_veiculos.Bitmap(((x * 15) + y) * 2));
				// Este calculo { ((x * 15) + y) } informa o m_tipo de construcao
		...
		m_ladrilho_sel_direito.SetarX(LADRILHO_SEL_DIREITO_X);
		m_ladrilho_sel_direito.SetarY(LADRILHO_SEL_DIREITO_Y);
		m_tela.RectFill(eCamadaFundo, LADRILHO_SEL_DIREITO_X, LADRILHO_SEL_DIREITO_Y, LADRILHO_SEL_DIREITO_X + LADRILHO_LARGURA, LADRILHO_SEL_DIREITO_Y + LADRILHO_LARGURA, makecol(0, 0, 0));
		m_tela.Rect(eCamadaFundo, LADRILHO_SEL_DIREITO_X, LADRILHO_SEL_DIREITO_Y, LADRILHO_SEL_DIREITO_X + LADRILHO_LARGURA, LADRILHO_SEL_DIREITO_Y + LADRILHO_LARGURA, makecol(100, 100, 100));
		m_ladrilho_sel_direito.Desenhar(m_tela, 0, 0);
		m_flag_desenhar = true;
	}
}
...
void CEditor::CarregarConfiguracao()
{
	...
	m_tela.Iniciar(modo_video, 640, 480, 16);
	...
	CExplosao::Iniciar(ARENA_A, ARENA_L, m_cfg.ObterBool("video", "particulas"));
}
